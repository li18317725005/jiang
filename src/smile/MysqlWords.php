...
class MysqlWords
{
    ...
    // 获取全部关键字
    public static function getWords($type = 'string', $state = 'lower')
    {
        if ('lower' == $state) {
            self::$words = array_map('strtolower', self::$words);
        }
        switch ($type) {
            case 'string':
    ...
    // 判断字符串知否在关键词内
    public static function inWords($word)
    {
        if (empty($word)) {
            exit('请填写参数word');
        }
